...
class Loss_vit(torch.nn.Module):
    ...
    def forward(self, outputs, source,out_prev=None,use_dir=True,target=None,target_ca=None,frac_cont=1.0,mask=None):
        losses = {}
        losses_val = {}
        loss_G = 0
        ...
        if target is not None:
            if self.lambdas['lambda_trg'] > 0:
                if mask == None:
                    ...
                    losses['loss_trg'] = self.calculate_target_loss(outputs*mask, target)

                losses_val['loss_cls'] = losses['loss_trg']
                loss_G += losses['loss_trg'] * self.lambdas['lambda_trg'] * target_ca
    ...
    def classify(self, inputs):
        ...
        for a in zip(inputs):
            # a = (3,224,224)
            a = self.global_transform(inputs).to(device)
            with torch.no_grad():
                cls_vector = self.extractor.get_cls_info(a)
                tensor = cls_vector
        return tensor
